...
class AuditEntry:
    ...
    decision: str           # "approved" | "rejected" | "auto_accepted" | "accepted_via_accept_all" |
                            ...
                            #  no bridge involved. The four decision strings above and
                            #  "bridge_proposal_no_op" below are deliberately NOT renamed to match:
                            #  they're already written, unversioned, into every existing install's
                            #  logs/audit/*.jsonl history, and any consumer of that history (the
                            #  weekly Excel export's own PatternFill lookup below, a maintainer's ad
                            #  hoc log grep, a future analytics pass) matches on the literal string.
