...
def _ensure_std_streams() -> None:
    ...
    daemon, a console window flashing up at every sign-in would be a bug of
    its own), and a windowed process started with no console at all has no
    standard handles. CPython then sets ``sys.stdout``/``sys.stderr`` to
    ...
    own logging, whose default formatter does ``sys.stdout.isatty()`` to
    decide about colour. So the daemon died here, every time, with
    ...
    itself, and the same would happen to the installer's own "launch
    ...
    ``windows-graphical-session.yml`` had Task Scheduler start it -- the
    packaged smoke tests included -- launched it from a shell with stdout
    ...
    daemon's own logging goes to its log file regardless (``setup_logging``),
    and a background process has nowhere meaningful to write a console
    stream to anyway.
    """
    devnull = None
