...
def build_parser() -> argparse.ArgumentParser:
    ...
    mobile_relay.add_argument(
        ...
        help="Explicitly turn mobile relay approval back off (useful with --merge).",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not services and "unattended_sessions" not in bundle and "mobile_relay" not in bundle:
        raise SystemExit(
            "No service or --enable/disable-unattended-sessions/--*-mobile-relay flags "
    ...
    if "mobile_relay" in bundle:
        summary += f", mobile_relay.enabled={bundle['mobile_relay']['enabled']}"
