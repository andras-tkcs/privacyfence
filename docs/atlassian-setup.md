...
Either the `read:attachment:confluence`/`readonly:content.attachment:confluence` scopes weren't added to the OAuth app yet (see step 3 above), or they were, but existing users haven't clicked **Reconnect…** on Confluence yet — a token issued before these scopes existed keeps whatever scopes it was issued with. Add the scopes if missing, then have affected users **Reconnect…**.
