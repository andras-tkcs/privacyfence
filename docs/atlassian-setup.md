...
**`scripts/qa_fixture_recorder.py --lifecycle`'s Confluence cleanup fails with `Unauthorized (401)`** (whoever runs the connector live check)
Same cause as the attachment case above, for `delete:page:confluence`: either the scope isn't added to the OAuth app yet (see step 3), or the QA account's stored token predates it. Add the scope if missing, then re-run the Atlassian OAuth step for the QA account (`docs/connector-live-check-setup.md` Phase A.3/B.3) to get a token that has it — the create/get/update steps in the same check all succeed regardless, since they only need the write/read scopes the token already had.
