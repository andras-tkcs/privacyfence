...
- **Confluence API** — add these as **granular** scopes: `read:space:confluence`, `read:page:confluence`, `write:page:confluence`, `read:content:confluence`, `read:content-details:confluence`, `write:content:confluence`. Confluence Cloud's newer v2 API (which PrivacyFence uses to list spaces) only accepts granular-scoped tokens — a classic-scoped token 401s on those endpoints ("scope does not match") even with `read:confluence-space.summary` granted. `read:content-details:confluence` gates CQL/text search and content-detail reads; `write:content:confluence` is needed for page creation alongside `write:page:confluence`. (Note: `search:confluence` is a **classic**-only scope name and won't appear in the granular picker — don't look for it there.)
