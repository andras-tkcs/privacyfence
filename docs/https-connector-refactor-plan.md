...
| **D12** | A later branch tried P4c — a "Connect Claude" card on `/settings` showing the `/mcp` URL and bearer token, in place of P4b's shim — then reverted it back to P4b. Worth it? | **No — stay on P4b.** P4c's whole value only exists for a client the user can already point at an arbitrary URL+token themselves; on a Team-plan `claude.ai` account there is no such path at all — adding a custom MCP connector is an org/Enterprise capability, not something a Team seat can self-serve. Until P10 ships `org` mode and that capability opens up at the org level, a Team-plan `local`-mode install has nothing to gain from the card and Desktop still needs P4b's shim regardless. Revisit P4c once P10 lands. |
