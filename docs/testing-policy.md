...
| 6 | Packaged-artifact | Installer/package correctness | Partially built. `test_macos_packaged_smoke.py` runs in `build.yml`'s tag-triggered release path; `test_deb_packaged_lifecycle.py` (Phase 6.3, done) now runs the Linux `.deb` install/validate/remove/purge and upgrade-in-place lifecycles (`linux-local-deb-packaging-plan.md` P7.1/P7.3) the same way, in `build.yml`'s `build-deb` job; `test_windows_packaged_smoke.py` (Phase 6.2, done) now runs a real silent install/autostart-task-registration/uninstall lifecycle against the Windows installer the same way, in `build.yml`'s `build-windows` job — upgrade/state-preservation testing across all three platforms is a deliberate follow-up, not this item. | `packaged` (registered; applied to `test_deb_packaged_lifecycle.py` and `test_windows_packaged_smoke.py` — not yet to `test_macos_packaged_smoke.py`, which predates this marker work, same posture as `browser`'s own note above) |
...
has landed too. Layer 6 (packaged-artifact) doesn't have a settled row here yet — the
