...
`build.yml`'s run for the same commit succeeding (Phase 6.4), and Linux's own graphical-session/
autostart verification (Phase 7 item 1) is wired into its own scheduled `linux-graphical-session.yml`
workflow rather than `build.yml`, since a flaky run in this tier must never block a release.
...
`linux-graphical-session.yml`'s row is a different case: it does run on a GitHub-hosted
`ubuntu-latest` runner, same as `build.yml`, but never on `pull_request` (a fork PR can't trigger
it), and the one secret it bakes in (`TELEGRAM_API_ID`/`TELEGRAM_API_HASH`) is the same shared,
non-per-account app identity `build.yml` itself already bakes into every distributed release build
on a GitHub-hosted runner — not a live per-account third-party OAuth credential of the kind this
policy exists to keep off GitHub-hosted/`pull_request` infrastructure.
