...
upgrade tests are wired into `build.yml`, and `publish-pypi.yml`'s own publish steps are gated on
`build.yml`'s run for the same commit succeeding (Phase 6.4).
